#!/bin/bash
#SBATCH --job-name=baseline-calodif
#SBATCH --nodes=1
...
export ACTIVATE="module load python; conda activate calodif"

export CHECKPOINT_DIR=$HOME/CaloDiffusion/checkpoints
mkdir -p $CHECKPOINT_DIR
export CONFIG=$HOME/CaloDiffusion/baseline_config.json

export CALODIF_COMMAND="timeout 2.8h python3 $HOME/CaloDiffusion/calodiffusion/training.py -c $CONFIG -d $TRAIN_DATA --checkpoint $CHECKPOINT_DIR --hgcal --load diffusion"
